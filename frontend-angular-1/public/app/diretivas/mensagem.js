...
function addMensagemRetorno(ctrl, result) {
        ...
        if(result.data && result.data!=null){
            obj = result.data.resposta;
        }else{
            obj = result;
        }
        ctrl.mensagem.status = result.status;
        ctrl.mensagem.mensagens = obj.mensagens;

        if (ctrl.mensagem.status && ctrl.mensagem.status == 404) {
                ctrl.mensagem.mensagem = "404 - Recurso nao encontrado.";
        }
        else {
                if (!ctrl.mensagem.status || ctrl.mensagem.status == null) {
                        if(obj.mensagens!=null){
